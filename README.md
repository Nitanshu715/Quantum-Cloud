...
Encode — Cloud scheduling → QUBO matrix → Cost Hamiltonian $H_C$
Quantum solve — QAOA circuit explores the full assignment space simultaneously
Measure — Sample the lowest-energy bitstring = optimal job allocation
Deploy — Feed result to Amazon EKS / AWS Batch for real scheduling
```
...
$$B \cdot \left(\frac{\sum_i \text{cpu}i \cdot x{ij}}{\text{cap}_j}\right)^2 \quad \forall j$$
③ Cost objective — minimize total cost × utilization:
$$\sum_{i,j} \text{cost}_j \cdot (\text{cpu}_i + 0.1 \cdot \text{mem}i) \cdot x{ij}$$
...
Greedy energy ≈ −261, SA/Brute Force energy ≈ −352 (35% gap)
QAOA energy matches SA (quantum = classical optimum)
Bitstring distribution shows orange bar (optimal state) with highest probability
All methods show `Feasible: ✓`
---
📊 Benchmark Results
